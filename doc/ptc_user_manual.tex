...
section. This flag should be set once and only once, after the call to ptc\_solver\_\_clean\_up but prior to anything else.
      ...
      \item epsilon, where Value must be an integer between 0 and 1. The default is 0.001. This is used during the labeling of floats and indicates the minimum precision required. This is a very technical area and users are warned that requiring a very low minimum precision may drastically increase run time. This value is directly used by the underlying ECLiPSe IC library and works with the propagation treshold.  
     \end{itemize}
     }
...
\hspace{\parindent}\pro    {ptc\_solver\_\_variable\_range(Identifier, Min, Max)}
        {Returns the minimum and maximum of the range of an Integer or Real variable.
Based on the linear constraints imposed on the variable.}
        {ptc\_solver\_\_variable\_range(Child, 0, 21)}
        {}
...
\pro    {ptc\_solver\_\_label\_integers(Identifier\_list, ChoicePoint)}
...
\pro    {ptc\_solver\_\_label\_reals(Identifier\_list, Float\_list)}
