...
:- export ptc_enum__clean_up/0, ptc_enum__record_enum/2, ptc_enum__is_enum/1, ptc_enum__is_enum_type/1, ptc_enum__create_enum/5,
	  ptc_enum__sample/1, ptc_enum__get_position/3,
          ptc_enum__get_position/2, ptc_enum__get_literal/3, ptc_enum__get_literal/2,
...
ptc_enum__create_enum(Basetype, Subtype, First_pos, Last_pos,  E) :-
	Position :: First_pos..Last_pos, %constrain the position
...
ptc_enum__get_literal(Enum_type, Position, Literal) :-
	Position #= Position_eval + 0,          %evaluate the position
	!,
	ptc_enum(Enum_type, Enum_list),             %find the enumeration list
	!,
...
ptc_enum__succ(X{ptc_enum(BasetypeX, _, PositionX)}, R{ptc_enum(_, _, PositionR)}) :-
	...
	(nonground(PositionR) ->
	    (%we ensure that X is less than R
	     dvar_domain(PositionR, DomR),
	     dom_range(DomR, New_minR, New_maxR),
...
ptc_enum__pos(Basetype, X{ptc_enum(_, _, Position)}, R) :-
	...
	!,
	R = Position,
	make_suspension(ptc_enum__pos(Basetype, X, R), 2, Susp), %part of enum invariant so quick
	insert_suspension(R, Susp, any of fd, fd).
...
ptc_enum__pos(Basetype, X{ptc_enum(_, _, Position)}, R) :-
	...
	;
	    (R :: New_minR..New_maxR,
             make_suspension(ptc_enum__pos(Basetype, X, R), 2, Susp), %part of enum invariant so quick
	     insert_suspension(R, Susp, any of fd, fd) %indirectly is also suspended on the position of X
	    )
	).
